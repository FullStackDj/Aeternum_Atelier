    ...
    <div
      ...
      <div className='flex max-sm:items-center sm:flex-col pt-5'>
        <div className='flex sm:flex-col gap-x-5 gap-y-8 sm:pt-10'>
          <NavLink to={'/'}
                   className={({isActive}) => isActive ? 'active-link' : 'flexStart gap-x-2 p-5 ' +
                     'bold-15 text-secondary cursor-pointer max-w-60 h-10 rounded-xl'}>
            <FaSquarePlus/>
        ...
        <div className='max-sm:ml-5 sm:mt-80'>
          {token && (
            <button onClick={() => setToken('')}
                    className='flexStart gap-x-2 p-5 bold-15 text-red-500 cursor-pointer max-w-60
                    h-10 rounded-xl'>
