      ...
      <div className='flex flex-col sm:flex-row gap-8 mt-8 xl:mt-6'>
        <div className='min-w-60 bg-white p-4 rounded-2xl'>
            ...
            <div className='flex flex-col gap-2 text-sm font-light'>
              {["Men", "Women", "Kids"].map((cat) => (
                <label key={cat} className='flex gap-2 medium-14 text-gray-30'>
                  <input onChange={(e) => toggleFilter(e.target.value, setCategory)}
            ...
            <div className='flex flex-col gap-2 text-sm font-light'>
              {["Topwear", "Bottomwear", "Winterwear"].map((subCat) => (
                <label key={subCat} className='flex gap-2 medium-14 text-gray-30'>
                  <input onChange={(e) => toggleFilter(e.target.value, setSubCategory)}
          ...
          <select onChange={(e) => setSortType(e.target.value)}
                  className='medium-14 h-8 w-full border border-slate-900/5 bg-primary text-gray-30 rounded-lg
                  px-2 outline-none mt-6'>
